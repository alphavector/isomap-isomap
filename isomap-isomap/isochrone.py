...
gmaps = googlemaps.Client(key='AIzaSyDTwPj1UDZ3ghD0qz7cfN-FMgOoJMnby_8')
# from mapbox import Geocoder
# geocoder = Geocoder(access_token='pk.eyJ1IjoiYmlnZGFkZHliZWFyZCIsImEiOiJjamd2dGozYjAwNGU3MnhxaTdjenhxYXp5In0._HhVMM2od89vxbJjQZQj3w')
...
def build_url(origin='',
    ...
    prefix = 'https://maps.googleapis.com/maps/api/distancematrix/json?mode=transit&traffic_model=pessimistic&units=imperial&avoid=tolls|ferries'
    if access_type == 'personal':
        url = urlparse.urlparse('{0}&origins={1}&destinations={2}&key={3}'.format(prefix,
                                                                                  quote(origin_str),
                                                                                  destination_str,
                                                                                  key))
        return url.scheme + '://' + url.netloc + url.path + '?' + url.query
...
def geocode_address(address='',
    ...
    config.read('{}google_maps.cfg'.format(config_path))
    key = config.get('api', 'api_number')
    #print('addres is %s' % address)

    # Convert the URL string to a URL, which we cfan parse
    # using the urlparse() function into path and query
    # Note that this URL should already be URL-encoded
...
def sort_points(origin_geocode, origin='',
                iso='',
                access_type='personal',
                config_path='config/'):
    """
...
def get_isochrone(origin='',
    ...
    while sum([a - b for a, b in zip(rad0, rad1)]) != 0:
        ...
        for i in range(number_of_angles):
            iso[i] = select_destination(origin_geocode, phi1[i], rad1[i], access_type, config_path)
    ...
    for i in range(number_of_angles):
        ...
        #print('\n#########################\n',data,"\n\n", iso[i], '\n#########################\n')
        time.sleep(0.1) 
    iso = sort_points(origin_geocode, origin, iso, access_type, config_path)
    return iso
...
def generate_isochrone_map(origin='',
    ...
    origin_geocode = geocode_address(origin, access_type, config_path)
    iso = get_isochrone(origin, duration, number_of_angles, tolerance, origin_geocode, access_type, config_path)
    ...
    <script src="https://maps.googleapis.com/maps/api/js?v=3.exp&key=AIzaSyCGRGw4ROCRy-A3VOfNYOlDA5L3qj9DQ2o"></script>
    <script>
    function initialize() {{
      var mapOptions = {{
        zoom: 14,
    ...
    return iso, '{}.html'.format(file_name)
